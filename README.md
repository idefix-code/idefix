...
Optional xdmf(hdf5+xmf) file dumping feature has been added to `Idefix`. This uses either serial or parallel implementation of `hdf5` library which needs to be made available. These xdmf file pair can be easily visualized in `ParaView` or `VisIt` and can also be loaded in `python` (using `h5py`) for post-processing and post-run analysis. One can turn on `xdmf` data dumps by using -DIdefix_HDF5=ON
