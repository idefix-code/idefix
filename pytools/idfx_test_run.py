...
#
# Source file pytools/idfx_test_run.py
#
...
class IdexPytestRunner:
    ...
    def genTests(self) -> list:
        ...
        with moveInDir(sourceDir):
            ...
            for testfile in testfiles:
                try:
                    # calc some paths
                    testfileRelPath = os.path.relpath(
                        testfile, os.path.join(sourceDir, "test")
                    )
                    testfilePath = os.path.abspath(
                        os.path.join("test", testfileRelPath)
                    )
                    ...
                    with open(testfilePath, "r") as fp:
                        ...
                        if "namings" in test:
                            ...
                            autoExtracted = idefixTestGenerator.extractNamingParameters(
                                variants
                            )
                            self._validateNaming(namings, autoExtracted, testfilePath)
                        else:
                            namings = idefixTestGenerator.extractNamingParameters(
                                variants
                            )
                        ...
                        # gen
                        result += idefixTestGenerator.genTestConfigs(
                            namings,
                            variants,
    ...
    def run(self, config: dict) -> None:
        ...
        with moveInDir(problemDir):
            self._runNonRegression(
                ...
                nonReg=nonRegressionTest,
                nonRegIni=nonRegressionTestIni,
            )
    ...
    def _runNonRegression(
        ...
        standardTest=True,
        first_run_ini=None,
        first_run_dumpname=None,
        configure_and_compile=True,
    ):
        if "multirun" in config_override:
            self._runNonRegMultirun(
                dumpname,
                ini,
                config_override,
                tolerance=tolerance,
                ...
                nonRegIni=nonRegIni,
                standardTest=standardTest,
                configure_and_compile=configure_and_compile,
                definitionFile=definitionFile,
                first_run_ini=first_run_ini,
                first_run_dumpname=first_run_dumpname,
            )
        else:
            # single basic run
            self._runNonRegSingleRun(
                dumpname,
                ini,
                config_override,
                tolerance=tolerance,
    ...
    def _runNonRegSingleRun(
        ...
        if configure_and_compile:
            idefixTest.configure(
                override=config_override, definitionFile=definitionFile
            )
            idefixTest.compile()
        ...
        if first_run_ini:
            ...
            if nonReg:
                idefixTest.nonRegressionTest(
                    filename=first_run_dumpname, tolerance=tolerance
                )
        ...
        if idefixTest.all:
            status = pytest.main(
                [
                    "-v",
                    "--no-header",
                    "--junit-xml=idefix-tests.junit.xml",
                    "--tb=short",
                ]
