...
#
# Source file pytools/tests/test_idfx_test_gen.py
#
...
def test_genNextLevelCombinations():
    ...
    assert lst1 == [{"mpi": True}, {"mpi": False}]
...
def test_genOneConfigSeries():
    ...
    # gen
    result = gen._genOneConfigSeries(
        "mpi,name", {"mpi": [True, False], "name": ["a", "b"]}
    )
...
def test_applyWhen_none():
    ...
    # call when clause
    res = gen._applyWhen({"a": 10, "b": 11}, when={})
...
def test_gen_full():
    ...
    # gen
    result = gen.genTestConfigs(
        "mpi,name",
        {"mpi": [True, False], "name": ["a", "b"]},
        {
            "conditions": {"mpi": True},
            "apply": {"extra": 10},
        },
    ...
    assert result == [
        ...
        pytest.param(
            {"mpi": False, "name": "a", "testfile": __file__, "testname": "unit-test"},
            id="unit-test-a",
        ),
        pytest.param(
            {"mpi": False, "name": "b", "testfile": __file__, "testname": "unit-test"},
            id="unit-test-b",
        ),
    ]
