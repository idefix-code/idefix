...
class ExtrapolateToFaces {
 ...
 public:
  explicit ExtrapolateToFaces(RiemannSolver<Phys> *rSolver):
    ...
    if constexpr(RiemannSolver<Phys>::preComputeSlopes) {
      PrimL = rSolver->PrimL;
      PrimR = rSolver->PrimR;
    }
  }
  ...
  KOKKOS_FORCEINLINE_FUNCTION void ExtrapolatePrimVar(const int i,
    ...
    [[maybe_unused]] constexpr int ioffset = (dir==IDIR ? 1 : 0);
    [[maybe_unused]] constexpr int joffset = (dir==JDIR ? 1 : 0);
    [[maybe_unused]] constexpr int koffset = (dir==KDIR ? 1 : 0);
