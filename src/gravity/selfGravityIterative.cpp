...
void SelfGravityIterative::Init(Input &input, DataBlock *datain) {
  idfx::pushRegion("SelfGravityIterative::Init");
  ...
  for(int dir = 0 ; dir < DIMENSIONS ; dir++) {
    ...
    if(boundary.compare("nullpot") == 0) {
      ...
      msg << "SelfGravityIterative:: Unknown boundary type " << boundary;
      IDEFIX_ERROR(msg);
    }
  ...
  if(solver == BICGSTAB || solver == PBICGSTAB) {
    iterativeSolver = std::make_unique<Bicgstab<Laplacian>>(*laplacian.get(), targetError, maxiter,
                                              laplacian->np_tot, laplacian->beg, laplacian->end);
  } else if(solver == CG || solver == PCG) {
    iterativeSolver = std::make_unique<Cg<Laplacian>>(*laplacian.get(), targetError, maxiter,
                                        laplacian->np_tot, laplacian->beg, laplacian->end);
  } else if(solver == MINRES || solver == PMINRES) {
    iterativeSolver = std::make_unique<Minres<Laplacian>>(*laplacian.get(),
                                  targetError, maxiter,
                                  laplacian->np_tot, laplacian->beg, laplacian->end);
  } else {
...
void SelfGravityIterative::ShowConfig() {
  ...
  switch(solver) {
    ...
    case PMINRES:
      ...
      break;
    default:
      IDEFIX_ERROR("SelfGravityIterative:: Unknown solver");
  }
...
void SelfGravityIterative::InitSolver() {
  idfx::pushRegion("SelfGravityIterative::InitSolver");
...
void SelfGravityIterative::SubstractMeanDensity() {
  idfx::pushRegion("SelfGravityIterative::SubstractMeanDensity");
...
void SelfGravityIterative::EnrollUserDefBoundary(Laplacian::UserDefBoundaryFunc myFunc) {
  laplacian->EnrollUserDefBoundary(myFunc);
}



void SelfGravityIterative::SolvePoisson() {
  idfx::pushRegion("SelfGravityIterative::SolvePoisson");
  ...
  if (this->nsteps<0) {
    idfx::cout << "SelfGravityIterative:: BICGSTAB failed, resetting potential" << std::endl;
    ...
    if (this->nsteps<0) {
      IDEFIX_ERROR("SelfGravityIterative:: BICGSTAB failed despite restart");
    }
  }
...
void SelfGravityIterative::AddSelfGravityPotential(IdefixArray3D<real> &phiP) {
  idfx::pushRegion("SelfGravityIterative::AddSelfGravityPotential");
