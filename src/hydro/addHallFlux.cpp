...
void Hydro::AddHallFlux(DataBlock &data, int dir, const real t) {
  ...
  if(haveHall == UserDefFunction && dir == IDIR) {
    if(hallDiffusivityFunc)
      hallDiffusivityFunc(data, t, xHallArr);
    else
    ...
    KOKKOS_LAMBDA (int k, int j, int i) {
      ...
      if(dir == IDIR) {
        ...
        if(haveHall == UserDefFunction)
          xH = AVERAGE_3D_X(xHallArr,k,j,i);
      }
      if(dir == JDIR) {
        ...
        if(haveHall == UserDefFunction)
          xH = AVERAGE_3D_Y(xHallArr,k,j,i);             
      }
      if(dir == KDIR) {
        ...
        if(haveHall == UserDefFunction)
          xH = AVERAGE_3D_Z(xHallArr,k,j,i);  
      }
      ...
      // We now compute the fluxes on both sides of the cells,
      // and use an HLL solver to get the riemann flux
      ...
      // TWO beacuse the CFL for Hall is like a second order diffusion operator,
      // for which there's a factor 2
      cMax(k,j,i) += TWO_F*TWO_F*SR;
