...
void Hydro::CalcNonidealEMF(DataBlock &data, real t) {
    ...
    idefix_for("CalcNIEMF",
                ...
                KOKKOS_LAMBDA (int k, int j, int i) {
                    ...
                    if(haveResistivity == Constant)   eta = etaConstant;
                    if(haveAmbipolar == Constant)     xA = xAConstant;
                    ...
                    // Ohmic resistivity
                    if(haveResistivity == UserDefFunction) eta = AVERAGE_3D_YZ(etaArr,k,j,i);
                    if(haveResistivity) ex(k,j,i) += eta * Jx1;

                    // Ambipolar diffusion
                    if(haveAmbipolar == UserDefFunction) xA = AVERAGE_3D_YZ(xAmbiArr,k,j,i);
                    if(haveAmbipolar) {
                    ...
                    // Ohmic resistivity
                    if(haveResistivity == UserDefFunction) eta = AVERAGE_3D_XZ(etaArr,k,j,i);
                    if(haveResistivity) ey(k,j,i) += eta * Jx2;

                    // Ambipolar diffusion
                    if(haveAmbipolar == UserDefFunction) xA = AVERAGE_3D_XZ(xAmbiArr,k,j,i);
                    if(haveAmbipolar) {
                    ...
                    // Ohmic resistivity
                    if(haveResistivity == UserDefFunction) eta = AVERAGE_3D_XY(etaArr,k,j,i);
                    if(haveResistivity) ez(k,j,i) += eta * Jx3;

                    // Ambipolar diffusion
                    if(haveAmbipolar == UserDefFunction) xA = AVERAGE_3D_XY(xAmbiArr,k,j,i);
                    if(haveAmbipolar) {
