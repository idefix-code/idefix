...
void Hydro::CalcParabolicFlux(DataBlock &data, int dir, const real t) {
    ...
    if(haveResistivity == UserDefFunction && dir == IDIR) {
        if(ohmicDiffusivityFunc) ohmicDiffusivityFunc(data, t, etaArr);
        else IDEFIX_ERROR("No user-defined Ohmic diffusivity function has been enrolled");
    }
    if(haveAmbipolar == UserDefFunction && dir == IDIR) {
        if(ambipolarDiffusivityFunc) ambipolarDiffusivityFunc(data, t, xAmbiArr);
        else IDEFIX_ERROR("No user-defined ambipolar diffusivity function has been enrolled");
    }
    ...
    idefix_for("CalcParabolicFlux",data.beg[KDIR],data.end[KDIR]+koffset,data.beg[JDIR],data.end[JDIR]+joffset,data.beg[IDIR],data.end[IDIR]+ioffset,
                KOKKOS_LAMBDA (int k, int j, int i) 
            {
                ...
                if(dir==IDIR) {
                    ...
                    if(haveAmbipolar == UserDefFunction) xA = AVERAGE_3D_X(xAmbiArr,k,j,i);
                    if(haveAmbipolar) {
                ...
                if(dir==JDIR) {
                    ...
                    if(haveAmbipolar == UserDefFunction) xA = AVERAGE_3D_Y(xAmbiArr,k,j,i);
                    if(haveAmbipolar) {
                ...
                if(dir==KDIR) {
                    ...
                    if(haveAmbipolar == UserDefFunction) xA = AVERAGE_3D_Z(xAmbiArr,k,j,i);
                    if(haveAmbipolar) {
