    ...
    idefix_for("CalcRightHandSide",data.beg[KDIR],data.end[KDIR],data.beg[JDIR],data.end[JDIR],data.beg[IDIR],data.end[IDIR],
        KOKKOS_LAMBDA (int k, int j, int i) {
            ...
            if(needPotential) {
                switch(dir) {
                  case IDIR:
                    rhs[MX1] -= dt/dl * Vc(RHO,k,j,i) * (phiP(k,j,i+1) - phiP(k,j,i));      // Gravitational force in direction i
                    break;
                  #if DIMENSIONS >=2
                  case JDIR:
                    rhs[MX2] -= dt/dl * Vc(RHO,k,j,i) * (phiP(k,j+1,i) - phiP(k,j,i));      // Gravitational force in direction j
                    break;
                  #endif
                  #if DIMENSIONS == 3
                  case KDIR:
                    rhs[MX3] -= dt/dl * Vc(RHO,k,j,i) * (phiP(k+1,j,i) - phiP(k,j,i));      // Gravitational force in direction k
                    break;
                  #endif
                }
