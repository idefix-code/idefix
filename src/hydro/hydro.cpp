...
// --------------------------------------------------------
                        ...
                        if (sx == 0) {
                            ez(k,j,i)   += HALF_F*(ezj(k,j,i) - Ex3(k,j,i) + ezj(k,j,i+1) - Ex3(k,j,i+1));
                            ez(k,j-1,i) -= HALF_F*(Ex3(k,j,i) - ezj(k,j-1,i) + Ex3(k,j,i+1) - ezj(k,j-1,i+1));
    #if DIMENSIONS == 3
                            ey(k,j,i)   += HALF_F*(eyk(k,j,i) - Ex2(k,j,i) + (eyk(k,j,i+1) - Ex2(k,j,i+1));
                            ey(k-1,j,i) -= HALF_F*(Ex2(k,j,i) - eyk(k-1,j,i) + Ex2(k,j,i+1) - eyk(k-1,j,i+1));
    #endif
                        }
                        else {
                            ez(k,j,i)   += ezj(k,j,iu) - Ex3(k,j,iu);
                            ez(k,j-1,i) -= Ex3(k,j,iu) - ezj(k,j-1,iu);
    #if DIMENSIONS == 3
                            ey(k,j,i)   += eyk(k,j,iu) - Ex2(k,j,iu);
                            ey(k-1,j,i) -= Ex2(k,j,iu) - eyk(k-1,j,iu);
    #endif
                        }
                        ...
                        if (sy == 0) {
                            ez(k,j,i)   += HALF_F*(ezi(k,j,i) - Ex3(k,j,i) + ezi(k,j+1,i) - Ex3(k,j+1,i));
                            ez(k,j,i-1) -= HALF_F*(Ex3(k,j,i) - ezi(k,j,i-1) + Ex3(k,j+1,i) - ezi(k,j+1,i-1));
    #if DIMENSIONS == 3
                            ex(k,j,i)   += HALF_F*(exk(k,j,i) - Ex1(k,j,i) + exk(k,j+1,i) - Ex1(k,j+1,i));
                            ex(k-1,j,i) -= HALF_F*(Ex1(k,j,i) - exk(k-1,j,i) + Ex1(k,j+1,i) - exk(k-1,j+1,i));
    #endif
                        }
                        else {
                            ez(k,j,i)   += ezi(k,ju,i) - Ex3(k,ju,i);
                            ez(k,j,i-1) -= Ex3(k,ju,i) - ezi(k,ju,i-1);
    #if DIMENSIONS == 3
                            ex(k,j,i)   += exk(k,ju,i) - Ex1(k,ju,i);
                            ex(k-1,j,i) -= Ex1(k,ju,i) - exk(k-1,ju,i);
    #endif
                        }
    ...
    #if DIMENSIONS == 3
                        if (sz == 0) {
                            ex(k,j,i)   += HALF_F*(exj(k,j,i) - Ex1(k,j,i) + exj(k+1,j,i) - Ex1(k+1,j,i));
                            ex(k,j-1,i) -= HALF_F*(Ex1(k,j,i) - exj(k,j-1,i) + Ex1(k+1,j,i) - exj(k+1,j-1,i));
                            ey(k,j,i)   += HALF_F*(eyi(k,j,i) - Ex2(k,j,i) + eyi(k+1,j,i) - Ex2(k+1,j,i));
                            ey(k,j,i-1) -= HALF_F*(Ex2(k,j,i) - eyi(k,j,i-1) + Ex2(k+1,j,i) - eyi(k+1,j,i-1));
                        }
                        else {
                            ex(k,j,i)   += exj(ku,j,i) - Ex1(ku,j,i);
                            ex(k,j-1,i) -= Ex1(ku,j,i) - exj(ku,j-1,i);
                            ey(k,j,i)   += eyi(ku,j,i) - Ex2(ku,j,i);
                            ey(k,j,i-1) -= Ex2(ku,j,i) - eyi(ku,j,i-1);
                        }
    #endif
                        ...
                        exj(k,j,i) -= HALF_F*(Ex1(k,j,i) + Ex1(k,j+1,i));
                        exk(k,j,i) -= HALF_F*(Ex1(k,j,i) + Ex1(k+1,j,i));

                        eyi(k,j,i) -= HALF_F*(Ex2(k,j,i) + Ex2(k,j,i+1));
                        eyk(k,j,i) -= HALF_F*(Ex2(k,j,i) + Ex2(k+1,j,i));
   #endif
                        ezi(k,j,i) *= TWO_F;
                        ezj(k,j,i) *= TWO_F;
                        ezi(k,j,i) -= HALF_F*(Ex3(k,j,i) + Ex3(k,j,i+1));
                        ezj(k,j,i) -= HALF_F*(Ex3(k,j,i) + Ex3(k,j+1,i));
