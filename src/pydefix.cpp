...
py::array_t<real, py::array::c_style> GatherIdefixArray(IdefixHostArray3D<real> in,
  ...
  if(idfx::prank == 0) {
    for(int rank = 0 ; rank < idfx::psize ; rank++) {
      ...
      if(rank==0) {
        ...
        buf = in;
          ...
          idefix::MPI_Recv(np_tot, 3, MPI_INT, rank, 011, MPI_COMM_WORLD, &status);
          idefix::MPI_Recv(beg, 3, MPI_INT, rank, 012, MPI_COMM_WORLD, &status);
          idefix::MPI_Recv(gbeg, 3, MPI_INT, rank, 013, MPI_COMM_WORLD, &status);

          buf = IdefixHostArray3D<real>("pydefix::tempArray",
                                         np_tot[KDIR],np_tot[JDIR],np_tot[IDIR]);
          // Fetch data
          idefix::MPI_Recv(buf, np_tot[IDIR]*np_tot[JDIR]*np_tot[KDIR],
    ...
    if(keepBoundaries) {
      ...
      idefix::MPI_Send(np_int, 3, MPI_INT, 0, 010, MPI_COMM_WORLD);
      idefix::MPI_Send(np_tot, 3, MPI_INT, 0, 011, MPI_COMM_WORLD);
      idefix::MPI_Send(beg, 3, MPI_INT, 0, 012, MPI_COMM_WORLD);
      idefix::MPI_Send(gbeg, 3, MPI_INT, 0, 013, MPI_COMM_WORLD);
      idefix::MPI_Send(in, np_tot[IDIR]*np_tot[JDIR]*np_tot[KDIR], realMPI, 0, 014, MPI_COMM_WORLD);
    #else
      IDEFIX_ERROR("Can't deal with psize>1 without MPI.");
    #endif
  }
  ...
  #ifdef WITH_MPI
    if(broadcast) {
      idefix::MPI_Bcast(out, out.extent(0)*out.extent(1)*out.extent(2), realMPI, 0, MPI_COMM_WORLD);
    }
  #endif
