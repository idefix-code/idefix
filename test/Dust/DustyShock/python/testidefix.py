...
#
# Source file test/Dust/DustyShock/python/testidefix.py
#
# Last modified : 07/2026
#
# Copyright(C) by :
# - Geoffroy Lesur <geoffroy.lesur@univ-grenoble-alpes.fr> (IPAG / CNRS - 2025)
...
if not args.noplot:
    ...
    plt.plot(
        V.x[::4], V.data["Dust0_VX1"][::4, 0, 0], "o", markersize=4, color="tab:orange"
    )
    plt.plot(x, M * sol.y[0, :], color="tab:orange")
    plt.plot(
        V.x[::4], V.data["Dust1_VX1"][::4, 0, 0], "o", markersize=4, color="tab:blue"
    )
    plt.plot(x, M * sol.y[1, :], color="tab:blue")
    plt.plot(
        V.x[::4], V.data["Dust2_VX1"][::4, 0, 0], "o", markersize=4, color="tab:green"
    )
...
error = (
    np.mean(
        np.fabs(V.data["Dust0_RHO"][istart:, 0, 0] - 1 / sol.y[0, :])
        + np.fabs(V.data["Dust1_RHO"][istart:, 0, 0] - 1 / sol.y[1, :])
        + np.fabs(V.data["Dust2_RHO"][istart:, 0, 0] - 1 / sol.y[2, :])
    )
    / 3
)
print("Error=%e" % error)
