...
#
# Source file test/MHD/HallWhistler/python/testidefix.py
#
# Last modified : 07/2026
#
# Copyright(C) by :
# - Geoffroy Lesur <geoffroy.lesur@univ-grenoble-alpes.fr> (IPAG/UGA/CNRS - 2020 - 2021)
...
r = 10 ** np.arange(
    np.floor(np.log10(np.amin(sp))), np.ceil(np.log10(np.amax(sp))), 0.1
)
...
print(
    "Theoretical whistler frequency=%g, numerical=%g, error=%g" % (f_w, f_wnum, error)
)
