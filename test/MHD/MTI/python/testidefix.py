...
#
# Source file test/MHD/MTI/python/testidefix.py
#
...
sigma_vx = (
    0.5 * (np.log(V["kinx"][20]) - np.log(V["kinx"][15])) / (V["t"][20] - V["t"][15])
)
sigma_vy = (
    0.5 * (np.log(V["kiny"][20]) - np.log(V["kiny"][15])) / (V["t"][20] - V["t"][15])
)
sigma_by = (
    0.5 * (np.log(V["by_2"][20]) - np.log(V["by_2"][15])) / (V["t"][20] - V["t"][15])
)
sigma_num = (sigma_vx + sigma_vy + sigma_by) / 3.0
...
def dispMTI(wcond, k, Pr):
    ...
    return np.array(
        [1, wcond + v, wcond * v + N_2 * kx_2 / k_2, K * wcond * -(wbuoy**2)]
    )
