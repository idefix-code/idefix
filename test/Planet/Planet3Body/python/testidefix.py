...
#
# Source file test/Planet/Planet3Body/python/testidefix.py
#
...
if plot_orbit:
    ...
    ax.set_title(
        r"m$_{\rm sat}=0$, e=%.1f, n/n$_s$=%s (a0=%.3f)"
        % (ecc0, resonance, separation(resonance=resonance)),
        pad=20,
    )
    ...
    ax2.scatter(
        theta0,
        (rpl0 - np.sqrt(planet0[1] ** 2 + planet0[2] ** 2)) / (rpl0),
        c="b",
        s=200,
    )
    ax2.scatter(
        theta1,
        (rpl1 - np.sqrt(planet1[1] ** 2 + planet1[2] ** 2)) / (rpl1),
        c="g",
        s=200,
    )
...
if (
    error_mean0 < 1.0e-10
    and error_dispersion0 < 1.0e-9
    and error_mean1 < 1.0e-10
    and error_dispersion1 < 1.0e-9
):
    print("SUCCESS!")
    sys.exit(0)
else:
